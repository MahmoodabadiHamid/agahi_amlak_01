...
def start(bot, update):
    update.message.reply_text('سلام، خسته نباشيد')
    for i in range(10):
        for j in range(12):
            print(str(i)+'   '+str(j))
...
def kick(channel,users,bot,update,c):
    ...
    response = requests.post(
                            ...
                            data={'chat_id': '@testchannel12365478954', 'text':'..'}
                            ).json()       
    try:
        for user in users:
            print('\n  '+user[6])
            id=user[1]
            db="soverflow - Copy.sqlite"
            dbconnect = sqlite3.connect(db)
            c = dbconnect.cursor()
            ...
            if (userStatusInChannel == 'member'):
                    if (payed==0):
                        if(allUsingDays > 2):
                            ...
                            response = requests.post(
                            url='https://api.telegram.org/bot{0}/{1}'.format(token, method),
                            data={'chat_id': '@testchannel12365478954', 'text':txt1}
                            ).json()
                        ...
                        if(payedUsingDays> 30):
                            ...
                            response = requests.post(
                            url='https://api.telegram.org/bot{0}/{1}'.format(token, method),
                            data={'chat_id': '@testchannel12365478954', 'text':txt1+txt2}
                            ).json()
                            #bot.kickChatMember(channel,id)
                            break
                        if(payedUsingDays == 30):
...
def kickUser(bot, update):
     ...
     try:
        ...
        for channel in channelAddress:
            Thread(target=kick,args=(channel,users,bot,update,c)).start()
       ...
       # i+=1
        #print(str(i))
         ...
         #           requests.post(
          #                  url='https://api.telegram.org/bot{0}/{1}'.format(token, method),
           #                 data={'chat_id': '-1001112930072', 'text':str(i)+' -'+'kicking users...' }
            #                ).json()
